...
static core_msg_t current_active_object;

#if defined(CORE_TASK_OBJ_LOG_ENABLE)
log_queue_t    log_task_dbg_object_queue;
static uint8_t task_dbg_active_obj_queue[LOG_QUEUE_OBJECT_SIZE];
#endif

#if defined(CORE_IRQ_OBJ_LOG_ENABLE)
...
__CORE_WEAK void task_irq_io_entry_trigger() {
...
/* function MUST-BE redefine */
__CORE_WEAK void task_irq_io_exit_trigger() {
#if defined(CORE_IO_IRQ_ANALYZER)
    FATAL("TK", 0x04);
#endif
}
...
void task_polling_create(task_polling_t* task_polling_tbl) {
    ...
    if (task_polling_tbl) {
        ...
        while (task_polling_tbl[idx].id != CORE_TASK_POLLING_EOT_ID) {
            idx++;
        }
...
void task_post(task_id_t des_task_id, core_msg_t* msg) {
    tcb_t* t_tcb;
...
uint8_t task_remove_msg(task_id_t task_id, uint8_t sig) {
    ...
    core_msg_t* f_msg = CORE_MSG_NULL; /* MUST-BE initialized CORE_MSG_NULL */
    core_msg_t* t_msg;

    if (task_id >= CORE_TASK_EOT_ID) {
        FATAL("TK", 0x05);
    }
    ...
    if (task_ready & t_tcb->mask) {
        ...
        while (t_msg != CORE_MSG_NULL) {
            /* check message task id and signal */
            if (t_msg->des_task_id == task_id && t_msg->sig == sig) {
...
void task_post_common_msg(task_id_t des_task_id, uint8_t sig, uint8_t* data,
                          uint8_t len) {
    core_msg_t* s_msg = get_common_msg();
    set_msg_sig(s_msg, sig);
    set_data_common_msg(s_msg, data, len);
    task_post(des_task_id, s_msg);
}

void task_post_dynamic_msg(task_id_t des_task_id, uint8_t sig, uint8_t* data,
                           uint32_t len) {
    core_msg_t* s_msg = get_dynamic_msg();
    set_msg_sig(s_msg, sig);
...
void task_entry_interrupt() {
    ...
    current_task_id = CORE_TASK_INTERRUPT_ID;
...
int task_run() {
    ...
    log_queue_init(&log_task_dbg_object_queue,
                   ...
                   (LOG_QUEUE_OBJECT_SIZE / sizeof(core_msg_t)),
                   sizeof(core_msg_t), mem_write, mem_read);
#endif

    /* init irq log queue */
#if defined(CORE_IRQ_OBJ_LOG_ENABLE)
    log_queue_init(&log_irq_queue, (uint32_t)irq_queue,
                   (LOG_QUEUE_IRQ_SIZE / sizeof(exception_info_t)),
                   sizeof(exception_info_t), mem_write, mem_read);
#endif
...
void task_polling_set_ability(task_id_t task_polling_id, uint8_t ability) {
    ...
    while (__task_polling_table->id < CORE_TASK_POLLING_EOT_ID) {
    ...
    if (__task_polling_table->id == CORE_TASK_POLLING_EOT_ID) {
        FATAL("TK", 0x07);
    }
}
...
void task_polling_run() {
    ...
    while (__task_polling_table->id < CORE_TASK_POLLING_EOT_ID) {
        ENTRY_CRITICAL();
        if (__task_polling_table->ability == CORE_ENABLE) {
            EXIT_CRITICAL();
...
void task_sheduler() {
    ...
    while ((t_task_new = LOG2LKUP(task_ready)) > t_task_current) {
        ...
        /* update current task id NOTE: current task id will be change when
...
#if defined(CORE_TASK_LOG_CONSOLE_ENABLE)
            {
                uint32_t exe_time;
                uint32_t wait_time;
                if (current_active_object.dbg_handler.start_exe >=
                    current_active_object.dbg_handler.start_post) {
                    wait_time = current_active_object.dbg_handler.start_exe -
                                current_active_object.dbg_handler.start_post;
                } else {
                    wait_time = current_active_object.dbg_handler.start_exe +
                                ((uint32_t)0xFFFFFFFF -
                ...
                if (current_active_object.dbg_handler.stop_exe >=
                    current_active_object.dbg_handler.start_exe) {
                    exe_time = current_active_object.dbg_handler.stop_exe -
                               current_active_object.dbg_handler.start_exe;
                } else {
                    exe_time = current_active_object.dbg_handler.stop_exe +
                               ((uint32_t)0xFFFFFFFF -
                ...
                xprintf(
                    "taskID: "
                    "%d\tmsgType:0x%x\trefCnt:%d\tsig:%d\t\twaitTime:%"
                    "d\texeTime:%d\n",
                    current_active_object.des_task_id,
                    (current_active_object.ref_count & CORE_MSG_TYPE_MASK),
                    (current_active_object.ref_count & CORE_MSG_REF_COUNT_MASK),
                    current_active_object.sig, (wait_time), (exe_time));
            }
#endif
        }
        ...
        if (get_msg_ref_count(t_msg) > 1) {
            t_msg->dbg_handler.start_post =
                current_active_object.dbg_handler.stop_exe;
            t_msg->dbg_handler.start_exe = 0;
            t_msg->dbg_handler.stop_exe  = 0;
        }

        /* clear current active object */
        memset(&current_active_object, 0, sizeof(core_msg_t));
...
task_id_t task_self() { return current_task_info.id; }

task_id_t get_current_task_id() { return current_task_id; }

task_t* get_current_task_info() { return (task_t*)&current_task_info; }

core_msg_t* get_current_active_object() {
    return (core_msg_t*)&current_active_object;
}
