...
typedef void (*pf_task)(core_msg_t*);
...
extern void    task_post(task_id_t des_task_id, core_msg_t* msg);
extern void    task_post_pure_msg(task_id_t des_task_id, uint8_t sig);
extern void    task_post_common_msg(task_id_t des_task_id, uint8_t sig,
                                    uint8_t* data, uint8_t len);
extern void    task_post_dynamic_msg(task_id_t des_task_id, uint8_t sig,
                                     uint8_t* data, uint32_t len);
extern uint8_t task_remove_msg(
    task_id_t task_id, uint8_t sig); /* return numbers message's removed */
...
extern void task_entry_interrupt(); /* this function MUST-BE called when entry
                                       interrupt handler */
extern void task_exit_interrupt();  /* this function MUST-BE called when exit
                                       interrupt handler */

extern task_id_t task_self(); /* get current task id, except interrupt task */
extern task_id_t
                   get_current_task_id(); /* get current task id include interrupt task */
extern task_t*     get_current_task_info();     /* get current task info */
extern core_msg_t* get_current_active_object(); /* get current active object
                                                 info (active message) */
